...
if (!class_exists('bulk_password_reset')) {
    class bulk_password_reset {
        ...
        function __construct(){
            ...
            load_textdomain($this->hook, $mo);
						...
						if(is_admin()) {
							...
							// admin actions/filters for bulk_actions user menu
							add_action('admin_footer',          array(&$this, 'bpr_cust_bulk_admin_footer'));
							add_action('load-edit.php',         array(&$this, 'bpr_cust_bulk_action'));
							add_action('load-users.php',        array(&$this, 'bpr_cust_bulk_action'));
							add_action('admin_notices',         array(&$this, 'bpr_cust_bulk_admin_notices'));
						}
         ...
         */
                                ...
                                if (empty($recipients)) {
                                    ...
                                    foreach ($recipients as $recipient) {
                                        ...
                                        if ( $options['hardreset'] ) {
                                            $errors = $this->reset_password($recipient->id);
                                        } else {
                                            $errors = $this->sent_activation_mail($recipient->id);
                                        }
                                    }
                                ...
                                <form action="" method="post" id="bpr-config">
                                    ...
                                    <div id="advancedbprsettings" class="postbox">
                                        ...
                                        <div class="bprbox">
                                            <table class="form-table">
                                                <tr>
                                                    <th valign="top" scrope="row"><label for="hardreset"><?php _e('Direct password reset', 'bulk_password_reset') ?>:</label><br/><small><?php _e('Not recommended, as this immediately changes the password of the users.','bulk_password_reset') ?></small></th>
                 ...
                 */
                function sent_activation_mail($user_id) {
                    ...
                    //}
	                    ...
	                    $title = sprintf(__('[%s] Password Reset Request', 'bulk_password_reset'), $blogname);
                ...
                function reset_password($user_id) {
                    ...
                    if ( $options['passwordnag'] ) {
	                    ...
	                    if ( $message && !wp_mail($user->user_email, $title, $message) ) {
	                        ...
	                        $this->log_msg ('ERROR: Not able to sent reset password message to ' . $user->user_login);
	                    } else {
	                        $this->log_msg ('INFO: Successfully sent reset password message to ' . $user->user_login);
	                    }
										}
		 ...
		 */
		function bpr_cust_bulk_admin_footer() {
        ...
        return;
				...
				?>
					<script type="text/javascript">
						jQuery(document).ready(function() {
							jQuery('<option>').val('perform_reset').text('<?php _e('Execute password reset', 'bulk_password_reset')?>').appendTo("select[name='action']");
							jQuery('<option>').val('perform_reset').text('<?php _e('Execute password reset', 'bulk_password_reset')?>').appendTo("select[name='action2']");
						});
					</script>
				<?php
		}
...
/**
		 ...
		 */
		function bpr_cust_bulk_action() {

				// get the action
				$wp_list_table = _get_list_table('WP_Users_List_Table');  // depending on your resource type this could be WP_Posts_List_Table, WP_Comments_List_Table, etc
				$action = $wp_list_table->current_action();
				
				$allowed_actions = array("perform_reset");
				...
				// this is based on wp-admin/users.php
				$sendback = remove_query_arg( array('resetted', 'deleted', 'users'), wp_get_referer() ); // ???????
				if ( ! $sendback )
				...
				switch($action) {
					case 'perform_reset':
						...
						foreach( $user_ids as $user_id ) {
							...
							if ( !$this->reset_password($user_id) )
								wp_die( __('Error resetting password of user.') );
			
							$resetted++;
						}
						
						$sendback = add_query_arg( array('resetted' => $resetted, 'ids' => join(',', $user_ids) ), $sendback );
					break;
					
					default: return;
				}
				
				$sendback = remove_query_arg( array('action', 'action2', 'new_role', '_status', 'bulk_edit'), $sendback );
		...
		/**
		 * Step 3: display an admin notice on the user page after resetting password(s)
		 */
		function bpr_cust_bulk_admin_notices() {
			global $pagenow;
			
			if($pagenow == 'users.php' && isset($_REQUEST['resetted']) && (int) $_REQUEST['resetted']) {
				$message = sprintf( _n( 'Password reset has been sent.', '%s password resets have been sent.', $_REQUEST['resetted'] ), number_format_i18n( $_REQUEST['resetted'] ) );
